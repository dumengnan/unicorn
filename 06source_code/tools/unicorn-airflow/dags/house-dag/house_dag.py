...
task1 = BashOperator(task_id='CrawlHouseInfo',
                     bash_command=dag_config["task1_cmd"],
                     params=dag_config["task1_params"],
                     dag=dag)

task2 = BashOperator(task_id='analysis',
                     depends_on_past=False,
                     bash_command=dag_config["task2_cmd"],
                     dag=dag)

task3 = BashOperator(task_id='sendEmail',
                     depends_on_past=False,
                     bash_command=dag_config["send_email_cmd"],
                     dag=dag)
